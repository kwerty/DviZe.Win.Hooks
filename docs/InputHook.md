...
| :--               | :--                   | :--                               | :--
| `Hooks`           | `InputHookTypes`      | -                                 | `InputHookTypes.Keyboard` and/or `InputHookTypes.Mouse`.
| `InstallOnDemand` | `bool`                | `true`                            | Set to `false` to manage installation manually via `InstallAsync`.
| `OnDemandOptions` | `OnDemandOptions`\*\* | `OnDemandOptions.Default`         | Controls the session lifecycle when `InstallOnDemand` is `true`.

\* Defined in `Kwerty.DviZe.Win` ([DviZe.Win.Common](https://github.com/kwerty/DviZe.Win.Common)).\
\*\* Defined in `Kwerty.DviZe.Workers` ([DviZe.Common](https://github.com/kwerty/DviZe.Common)).
...
public Task<IDisposable> AddHandlerAsync(InputHandler handler, int priority, CancellationToken cancellationToken = default);
```
...
Dispose the returned `IDisposable` to uninstall the handler.
...
Uninstalls all handlers and hooks and releases native resources.
