...
public sealed class InputHook : IAsyncDisposable
{
    ...
    public InputHook(InputHookOptions options, IThreadAccessor threadAccessor, ILoggerFactory loggerFactory)
    {
        ...
        sessionProvider = options.InstallOnDemand
            ? new OnDemand<InputHookSession>(options.OnDemandOptions, CreateSession, loggerFactory)
    ...
    /// <summary>
    /// Installs a native handler for processing keyboard and/or mouse events.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    /// <returns>
    /// An <see cref="IDisposable"/> which should be disposed to uninstall the handler.
    /// </returns>
    /// <exception cref="InvalidOperationException" />
    /// <exception cref="ObjectDisposedException" />
    public Task<IDisposable> AddHandlerAsync(InputHandler handler, CancellationToken cancellationToken = default)
        => AddHandlerAsync(handler, priority: 0, cancellationToken);
    ...
    /// <exception cref="InvalidOperationException" />
    /// <exception cref="ObjectDisposedException" />
    public async Task<IDisposable> AddHandlerAsync(InputHandler handler, int priority, CancellationToken cancellationToken = default)
    {
        var registration = new InputHookRegistration(handler, priority, options, sessionProvider, threadAccessor);
    ...
    public Task<IDisposable> SubscribeAsync(Key key, Action<KeyboardEvent> callback, CancellationToken cancellationToken = default)
        => AddHandlerAsync(new DefaultKeyboardHandler(key, keyState: null, callback), cancellationToken);
    ...
    public Task<IDisposable> SubscribeAsync(Key key, KeyState keyState, Action<KeyboardEvent> callback, CancellationToken cancellationToken = default)
        => AddHandlerAsync(new DefaultKeyboardHandler(key, keyState, callback), cancellationToken);
    ...
    public Task<IDisposable> SubscribeAsync(MouseAction mouseAction, Action<MouseEvent> callback, CancellationToken cancellationToken = default)
        => AddHandlerAsync(new DefaultMouseHandler(mouseAction, callback), cancellationToken);
